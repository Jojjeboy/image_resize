...
dirBig = r"C:\Users\jokarn\image_resize"                                                             #source folder, absolute path
dirSmall = r"C:\Users\jokarn\image_resize_mini"                                                     #destination folder, absolute path
extList_img = [".jpg", ".jpeg", ".png", ".gif"]                                         #image file extensions
extList_vid = [".mp4", ".3gp", ".mov", ".avi", ".mpg", ".mpeg"]                         #video file extensions
newSize = (1024,0)                                                                       #set the dimensions for destination image. set one dimension to 0!
imgQuality = 90                                                                         #quality of new image
...
class work4me(threading.Thread):
    ...
    def shrink(self, args):
        ...
        if not os.access(file_to, os.F_OK) or not skipExisting:
            if os.path.splitext(file)[1] in extList_img:                  #image
                try:
                ...
                if (img_in.size[0]<10000 and img_in.size[1]<10000):
                    ...
                    count_error+=1
                    work4me.plock.release()
                if (debug and not logOnly): print("Press Enter to continue...")
            elif os.path.splitext(file)[1] in extList_vid:                #video
                if not os.access(file_to, os.F_OK) or not skipExisting:
                    shutil.copyfile(file_from, file_to)
                work4me.plock.acquire()
